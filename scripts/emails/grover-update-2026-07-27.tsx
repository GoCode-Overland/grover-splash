    ...
    <Container className="email-card" style={containerStyle}>
          ...
          <Section style={sectionStyle}>
            <Text className="intro-text" style={introStyle}>
              A few things landed in the app recently. Here's what actually changed for you.
            </Text>
          </Section>
          ...
          <Section style={cardOuterStyle}>
            <div className="box-card" style={cardStyle}>
              ...
              {/* Fire and smoke layers */}
              <Text className="item-title" style={itemTitleStyle}>New map layers: fire hotspots, fire perimeters, and smoke</Text>
              <Img
                src={`${BASE}/img/blog-photos/fire-hotspots-perimeters-map.jpeg`}
                alt="Grover map over Oregon showing fire hotspot icons and shaded red fire perimeter boundaries"
                width="260"
                style={heroPhotoStyle}
              />
              <Text className="item-body" style={itemBodyStyle}>
                Grover's map can now show active fire hotspots, fire perimeters, and smoke hazard areas, sourced from NASA FIRMS, NIFC/WFIGS, and NOAA, and refreshed several times a day. Hotspots are on by default. Perimeters and smoke are optional layers you can turn on from the map's Layers menu.
              </Text>
              <Text className="item-body" style={itemBodyStyle}>
                We built this because wildfire season is hard on the communities in its path, and we wanted vanlifers to have one more honest source of information when deciding whether to change a route. This is a planning aid, not an emergency alert. For anything urgent, please rely on local emergency management, InciWeb, or official road closure information first.
              </Text>
              <Link href={`${BASE}/blog/grover-fire-smoke-map-layers`} style={readMoreStyle}>Read more →</Link>

              <Hr className="card-divider" style={dividerStyle} />

              {/* Campflare availability + search, combined */}
              <Text className="item-title" style={itemTitleStyle}>Real Campflare availability, smarter search</Text>
              <Img
              ...
              />
              <Text className="item-body" style={itemBodyStyle}>
                Campflare campgrounds now show a real availability calendar, green for open, orange for walk-in only, red for full. And map search checks your pins, your Circle's pins, and Grover partner places too, putting the top 5 matches right at the top ahead of the usual map results. Type a name wrong and Grover still finds it.
              </Text>
              <Link href={`${BASE}/blog/grover-campflare-live-availability`} style={readMoreStyle}>Read more →</Link>
              {' '}
              <Link href={`${BASE}/blog/grover-map-search-app-update`} style={readMoreStyle}>Read about search →</Link>
...
export default function GroverUpdate20260727() {
  return (
    <Html lang="en">
      <Head>
        <style>{darkMode}</style>
      </Head>
      <Preview>New fire and smoke map layers, live campground availability, and your Circle.</Preview>
